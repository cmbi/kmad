...
void FeaturesProfile::get_score(unsigned int position, FeaturesList& features, 
  ...
  for (unsigned int i = 0; i < features.size(); i++){
       add_score += m_prf_matrix[features[i]][position];
  }
}


double FeaturesProfile::ScoreMotifs(unsigned int& position, std::string& feat_name){
			...
			for (unsigned int k = 0; k < features.size();k++){
				...
				if (!vec_util::CheckIfContains(list_of_features, feature_k) && feature_k != nothing){	//check whether this domain is already in the list of features
					...
					if (feature_k.substr(0,6) == "domain"){
						...
						motif_indexes.push_back(list_of_features.size()-1); // same as with domain_indexes
					}
				}
			}
		}
...
double FeaturesProfile::ScoreDomains(unsigned int& position, std::string& dom_name){
	double result = 0;
