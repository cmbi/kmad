...
//function performMSAfirstround - performs the first round of alignments, all vs query seq (first calculates profile based only on the query seq, then aligns all sequences and calculates identity of each sequence to the query seq.)
std::vector<std::string> Sequences::performMSAfirstround(Profile& outputProfile, FeaturesProfile& outputFeaturesProfile, double penalty, double endPenalty, double extensionPenalty, std::string verbose, bool weightsModeOn, int codon_length, std::vector<double>& identities){
	outputProfile = Profile(substitutionMatrix::convertToProfileFormat(sequences_aa[0])); 
	std::vector< std::vector<Residue> > alignmentWithoutLowercase;	//working alignment - without lowercase around cut out residues - would make latter aligning more complicated
	std::vector< std::vector<Residue> > alignmentWithLowercase;		//lowercase before and after cut out residues -- final result 
	...
	std::vector<Residue> alNoLower; //pairwise alignment without lowercase characters
	std::vector<Residue> alWithLower; //pairwise alignment with lowercase characters where chars were removed
	for (int i = 1; i < seqNr; i++){
		alignPairwise(alNoLower,alWithLower,sequences_aa[i],outputProfile,outputFeaturesProfile,penalty,endPenalty,extensionPenalty,i,verbose,codon_length,i);
...
void Sequences::performMSAnextRounds(std::vector<std::string>* prevAlignment, Profile& outputProfile,FeaturesProfile& outputFeaturesProfile, double penalty, double endPenalty, double extensionPenalty,std::string verbose, bool weightsModeOn, double identityCutoff,int codon_length, std::vector<double> identities, int& prev_alignments){
...
void Sequences::removeGaps(std::vector<Residue> &alignmentWithLowercase, std::vector<Residue> &alignmentWithoutLowercase, std::vector< std::vector<Residue> >& alignment){
	...
	for (unsigned int i = 0; i < alignment[0].size(); i++){
		...
		if (s1char == gap){
			if (newS2lower.size() > 0){
				newS2lower[newS2lower.size()-1].lowercase(); //change previous character to lowercase
			...
			if (lowerFlag){   //lowercase char
				Residue newRes = s2[i];
				newRes.lowercase();
				newS2lower.push_back(newRes); //add lowercase char to the alignment with lowercases
				newS2.push_back(s2[i]);      	//add uppercase alignment to the alignment without lowercases
				lowerFlag = false;
			}
			else{ 		//uppercase char
				newS2lower.push_back(s2[i]); // adds the same uppercase char to both alignments (with lowercases and without lowercases)
...
//function alignPairwise -> takes a sequence and profiles, returns a ready alignment of the two, with gaps cut out
void Sequences::alignPairwise(std::vector<Residue> &alNoLower,std::vector<Residue> &alWithLower,std::vector<Residue> seq2, Profile& prf, FeaturesProfile& featPrf, double penalty, double endPenalty, double extensionPenalty,int deb, std::string verbose, int codon_length, int sequence_no){
	int profileLength = prf.getMatrix()[0].size();
	std::vector< std::vector<Residue> > alignment;
	ScoringMatrix scores(profileLength,seq2.size(),penalty,endPenalty,extensionPenalty);
	scores.calculateScores(seq2, prf, featPrf,deb, codon_length, sequence_no);
	scores.nwAlignment(&alignment,seq2,prf,featPrf,verbose, codon_length, sequence_no);
